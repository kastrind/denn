...
module.exports = {
    ...
    NeuralNetwork: function(X, Y, formation) {
        return {
            input: X,
            Y: Y,
            output: 0,
            layers: [],
            layer_bak: null,
            next_weights_bak: null,

            initLayers: function(formation) {
                for (var i=0; i<formation.length; i++) {
                    if (i==0) this.layers.push({"weights": math.random([X[0].length, formation[i]]), "layer": 0});
                    else if (i<formation.length-1) this.layers.push({"weights": math.random([formation[i-1], formation[i]]), "layer": 0});
                    if (i==formation.length-1) this.layers.push({"weights": math.random([formation[i], Y[0].length])});
                }
                this.printLayers();
            },
            ...
            dropout_restore: function(layer_i) {
